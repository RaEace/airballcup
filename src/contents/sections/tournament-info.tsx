...
const getTournamentMeta = ({date}: Tournament) => ({
...
const TournamentInfo: FunctionComponent<{tournamentInfo: Tournament}> = ({tournamentInfo}) => {
    const info = getTournamentMeta(tournamentInfo);
    ...
    return (
        ...
        >
            <div className={"flex flex-col items-center md:items-start gap-4 sm:px-20"}>
                <div className={"flex flex-col items-center md:items-start gap-4"}>
                    ...
                    <h2 className={"smooth font-display font-bold uppercase lg:text-display-s md:text-title-l text-title-m"}>
                        {formatDateIntl(info.date, "fr-FR")}
                    </h2>
                </div>
            ...
            <div>
                <iframe
                    title={"Localisation de Belushi's Canal"}
...
function getLineImage(line: string, type: string): string {
    if (type === "metro") {
