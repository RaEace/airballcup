          ...
          <SheetContent side={"full"} className={"z-[101] bg-gray-950"}>
            <SheetTitle>
              <SheetClose>
                <a className={cn(buttonVariants({ variant: "link" }))} href="/">
                  <img src={airballCupLogo.src || "/placeholder.svg"} alt="AirballCup Logo" />
                </a>
              </SheetClose>
            </SheetTitle>
            <SheetDescription className={"mt-10"}>
              <div role={"navigation"} className={"flex flex-col items-start"}>
                {links.map(([href, title, target]) => (
                    <SheetClose key={href}>
                      <Link
                          ...
                          title={title}
                          target={target}
                          className={cn(buttonVariants({ variant: "link" }), "group font-display font-bold text-title-m")}
                          href={String(href)}
                      >
                        <ArrowRight
                            color={"#E51C21"}
                ...
                {isRankingsLoading ? (
                    <div className={cn(buttonVariants({ variant: "link" }), "font-display font-bold text-title-m")}>
                      Chargement...
                    </div>
                ) : (
                    availableRankings.length > 0 && availableRankings.map((season) => (
  ...
  return (
      ...
      >
        <Link prefetch title={"back-home-link"} href="/">
          <img src={airballCupLogo.src || "/placeholder.svg"} alt="logo airball cup" />
        </Link>
...
function Navbar({
  ...
  return (
      <NavigationMenu>
        <NavigationMenuList>
          <NavigationMenuItem>
            <NavigationMenuTrigger className={cn(buttonVariants({ variant: "link" }), "lg:text-button-m")}>
          ...
          <NavigationMenuItem>
            <NavigationMenuTrigger
                className={cn(buttonVariants({ variant: "link" }), "lg:text-button-m")}
            ...
            <NavigationMenuContent>
              {availableRankings && availableRankings.length > 0 ? (
                  <ul className={`w-[400px] grid grid-rows-${availableRankings.length} grid-cols-1`}>
                  ...
                  <div className="w-[400px] p-4 text-center">
                    {isRankingsLoading ? "Chargement des classements..." : "Aucun classement disponible"}
                  </div>
              )}
            </NavigationMenuContent>
          </NavigationMenuItem>
              ...
              <NavigationMenuItem key={href}>
                <NavigationMenuLink asChild>
                  <Link
                      ...
                      className={cn(buttonVariants({ variant: "link" }), "xl:text-button-m lg:text-[10px]")}
                      target={target}
                      href={String(href)}
                  >
                    {title}
